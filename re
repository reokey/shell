...
if [ -d "$dx_re" ];then
	rm -rf $dx_re
	if [ $? -eq 0 ]; then
		echo -e "${dco_p} 提示${dco_t}\t${dco_s}存在冲突文件夹 删除成功 ......${dco_t}"
	else
		echo -e "${dco_p} 提示${dco_t}\t${dco_s}存在冲突文件夹 删除失败 ......${dco_t}"
	fi
else
	if [ ! -f "$dx_re" ];then
		cp -f $0 $dx_re && chmod 777 $dx_re
			if [ $? -eq 0 ]; then
				echo -e "${dco_p} 提示${dco_t}\t${dco_s}RE未安装 自动安装成功 ......${dco_t}"
			else
				echo -e "${dco_p} 提示${dco_t}\t${dco_s}RE未安装 自动安装失败 ......${dco_t}"
			fi
	else
		echo "文件存在"
			...
			dx_re_n_md5=$(md5sum $dx_re_n|cut -d ' ' -f1)
				if [ $dx_re_md5 == $dx_re_n_md5 ]; then
					echo -e "${dco_p} 提示${dco_t}\t${dco_s}RE已安装 代码无需更新 ......${dco_t}"
				else
					echo -e "${dco_p} 提示${dco_t}\t${dco_s}RE已安装 代码需要更新 ......${dco_t}"
					cp -f $0 $dx_re && chmod 777 $dx_re
						if [ $? -eq 0 ]; then
							echo -e "${dco_p} 提示${dco_t}\t${dco_s}RE已安装 代码更新完成 ......${dco_t}"
						else
							echo -e "${dco_p} 提示${dco_t}\t${dco_s}RE已安装 代码更新失败 ......${dco_t}"
						fi
				fi
	fi
fi
