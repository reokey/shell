...
#设置颜色控制
export PS1="\e[33;31m\] ReOkey \e[33;36m" #天蓝 正常回显
...
if [[ -n $1 ]]
then
	echo -e "${dco_s}----------------------------------------------------${dco_t}" >> $RESULT_KEY
	echo -e "${dco_g}关键信息信息${dco_t}" >> $RESULT_KEY
	echo -e "${dco_s}----------------------------------------------------${dco_t}" >> $RESULT_KEY
	find / -maxdepth 4 -name *.conf -type f -exec grep -Hn $1 {} \; >> $RESULT_KEY 2>&1
	find / -maxdepth 4 -name *.ini -type f -exec grep -Hn $1 {} \; >> $RESULT_KEY 2>&1
	find / -maxdepth 4 -name *.log -type f -exec grep -Hn $1 {} \; >> $RESULT_KEY 2>&1
	if [[ $? -eq 0 ]]
	then
		echo >> $RESULT_KEY
...
update_dx_menu(){
	...
	case $answer in
		...
		-)
			...
			;;
		*)
			d_nver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/reokey/shell/master/dx"|grep 'd_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
...
menu_help(){
	...
	echo
	echo -ne "${dco_y}说明:\t [作用]...\t命令示例\t\t[说明]....${dco_t}\n\n\t ${dco_g}[帮助]\t\tre help\t\t\t[项目]\n\t [安装]\t\t${dco_r}bash re install${dco_t}\t\t${dco_g}[默认路径/usr/local/sbin/re]\n\t [清单]${dco_t}\t\t${dco_g}re list\t\t\t[无]\n\t [管理]\t\tre manage\t\t[授权]\n\t [脚本]\t\tre ok\t\t\t[项目]\n\t [扫描]\t\tre scan\t\t\t[项目]\n\t [更新]\t\tre update\t\t[项目]\n\t [版本]\t\tre ver\t\t\t[项目]${dco_t}\n\n"
}
...
#判断文件或者文件夹
if [ ! -e $dx_re ];then
	cp -f $0 $dx_re && chmod 777 $dx_re && exit
elif [ ! -e $dx_re_n ];then
...
if [ ! -f $dx_re ];then
    echo $dx_re 作为文件或文件夹是存在的
		if [ $dx_re_md5 == $dx_re_n_md5 ]; then
			...
			echo "命令不同"
			#echo -e "${dco_p} 提示${dco_t}\t${dco_s}系统已经支持re脚本 ${dco_p}[任意]${dco_t}${dco_s}键返回,${dco_t} ${dco_p}[0]${dco_t}${dco_s}重装${dco_t}"
		fi
fi
}
...
install_re111(){
	if [ ! -f "$dx_re" ]; then
		clear
		echo -e "${dco_p} 提示${dco_t}\t${dco_s}系统正在支持re脚本 ...... ${dco_t}"
		cp  $0 $dx_re && chmod 777 $dx_re
			if [ $? -eq 0 ]; then
				echo -e "${dco_p} 提示${dco_t}\t${dco_s}系统已经支持re脚本 ......${dco_t}"
			else
				echo -e "${dco_p} 提示${dco_t}\t${dco_s}系统不能正确支持re脚本,请联系作者.${dco_t}"
			fi
		exit 0
	else
		diff $dx_re $0 > /dev/null
		if [ $0 == 0 ]; then
		echo "命令不同"
			#cp -f $0 $dx_re && chmod 777 $dx_re
		else
		...
		0)
			echo -ne "\r     \r"
			echo -e "${dco_p} 提示${dco_t}\t${dco_s}系统正在重新支持re脚本 ...... ${dco_t}"
			cp -f re /usr/local/sbin/re && chmod 777 /usr/local/sbin/re
			if [ $? -eq 0 ]; then
			echo -e "${dco_p} 提示${dco_t}\t${dco_s}系统已经重新支持re脚本 ......${dco_t}"
			else
			echo -e "${dco_p} 提示${dco_t}\t${dco_s}系统不能正确重新支持re脚本,请联系作者.${dco_t}"
			fi
			exit 0
			;;
		*)
