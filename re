	...
	if [ ! -f "$dx_re" ];then
		...
		dx_re_n_md5=$(md5sum $dx_re_n|cut -d ' ' -f1)
			if [ $dx_re_md5 == $dx_re_n_md5 ]; then
				echo -e "${dco_s}当前数据\t[${dx_re_md5}]${dco_t}";
			else
				echo -e "${dco_s}需要升级\t[${dx_re_md5}]${dco_t}";
				rm -rf $dx_re
				cp -f $dx_re_n $dx_re
					if [ $? -eq 0 ]; then
						chmod 777 $dx_re
						echo -e "${dco_r}[更新完成]\t${dco_s}当前数据\t[${dx_re_n_md5}]${dco_t}";
					else
						echo -e "${dco_r}[更新失败]\t${dco_s}当前数据\t[${dx_re_md5}]${dco_t}";
					fi
			fi
	fi
fi
	...
	[[ -z ${d_nver} ]] && echo -e "${dco_r}[检测版本]${dco_s}\t最新\t[无法获取]${dco_t}" && exit 0
		if [[ ${d_nver} != ${dx_ver} ]]; then
			...
			case $answer in
				n)
					echo -ne "\r     \r"
					echo -e "${dco_r}[版本下载]${dco_s}\t取消\t[${dx_ver}]${dco_t}"
					;;
				*)
					echo -ne "\r     \r"
					echo -e "${dco_r}[版本下载]${dco_s}\t开始下载\t[${d_nver}]${dco_t}"
					rm -rf $dx_re_t
					wget -q -N -c -O $dx_re_t --no-check-certificate https://raw.githubusercontent.com/reokey/shell/master/re
					if [ $? -eq 0 ]; then
						rm -rf $dx_re $dx_re_n
						$dx_re_t
						cp -f $dx_re_t $dx_re
						cp -f $dx_re_t $dx_re_n
						rm -rf $dx_re_t
						chmod +x $dx_re $dx_re_n
...
menu_banner(){
	...
	echo -e "${dco_g}                 ______                 _                 \n                (_____ \               | |                  \n                 _____) ) _____   ___  | |  _  _____  _   _ \n                |  __  / | ___ | / _ \ | |_/ )| ___ || | | |\n                | |  \ \ | ____|| |_| ||  _ ( | ____|| |_| |\n                |_|   |_||_____) \___/ |_| \_)|_____) \__  |\n                                                     (____/ ${dco_t}"
	echo -e
}
...
menu_ver(){
	#menu_help_tips
	#menu_banner
	echo -e "${dco_g}┌──── 昨天的梦里 ─────────────────────────────────────── LINUX管理脚本 ────────┐\t│        小熊坐在狮子身上哭泣                                                  │\n│            他们告诉我                                 更新日期 [${dx_ver}]  │\n│        小熊坐在狮子身上哭泣                           ReOkey 唯一发布        │\n│            他们告诉我                                 https://t.me/daxihanze │\n│                那只蓝色的海豚                         如有修改请删除标识     │\n│                    也在梦里想念你                                            │\n└──── 至 秀一大人 ─────────────────────────────────────────────搬运工:达奚瀚泽─┘${dco_t}"
	echo -e
...
menu_help(){
	menu_banner
	echo -ne "${dco_y}说明:\t [作用]...\t命令示例\t\t[说明]....${dco_t}\n\n\t ${dco_g}[清单]${dco_t}\t\t${dco_g}re list\t\t\t[脚本列表]\n\t [管理]\t\tre manage\t\t[使用授权]\n\t [脚本]\t\tre ok\t\t\t[引导使用]\n\t [扫描]\t\tre scan\t\t\t[系统信息]\n\t [版本]\t\tre ver\t\t\t[脚本介绍]${dco_t}\n\n"
	update_re
	Check_re re;
}
...
#菜单命令
menu_gui(){
	echo -e "${dco_g}输入软件名首个字母\t\t${dco_t}"
	echo
	echo -e "\t${dco_y}说明:\t [作用]...\t命令示例\t\t[参数]....${dco_t}${dco_s}"
	echo -e"PS1='${debian_chroot:+($debian_chroot)}\[\033[01;35;40m\]\u\[\033[00;00;40m\]@\[\033[01;35;40m\]\h\[\033[00;31;40m\]:\[\033[00;00;40m\]\w \[\033[01;32;40m\]\$ \[\033[01;36;40m\]'
"
}
